$(document).on('turbolinks:load', function () {
  ...
  var resetPasswordPasswordConfirmation = $('.reset-password .password.confirmation');

  $('.reset-password input.password').on('propertychange change click keyup input paste', function (event) {
    if (resetPasswordPassword.val() === '' || resetPasswordPasswordConfirmation.val() === '') {
      ...
      if (resetPasswordPassword.val() === resetPasswordPasswordConfirmation.val()) {
        if (resetPasswordPassword.val().length < 6) {
          updateStatusMessage(resetPasswordStatusMessage, 'error', 'Le mot de passe doit être de plus de 6 caractères');
          resetPasswordSubmit.prop('disabled', true);
        } else {
          updateStatusMessage(resetPasswordStatusMessage);
        ...
        updateStatusMessage(resetPasswordStatusMessage, 'error', 'Les mots de passes ne correspondent pas');
        resetPasswordSubmit.prop('disabled', true);
      }
    }
  });
  ...
  .on('ajax:error', function (e, data, status, xhr) {
    updateStatusMessage(resetPasswordStatusMessage, 'error', data.responseJSON.message);
  })
